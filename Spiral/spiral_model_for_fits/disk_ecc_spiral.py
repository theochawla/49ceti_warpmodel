...
#        tempg = tmid*np.exp(np.log(tatm/tmid)*zcf/zq)
        ...
        bool_mask = (g_phi_tiled > -np.pi/12)*(g_phi_tiled < (2*np.pi + np.pi/12))
        ...
        interp_test = interpnd((g_r_tiled[bool_mask], g_phi_tiled[bool_mask]), spir_tiled[bool_mask])
        ...
        phi_vel = giggle.uphC(-gx, gy, self.ms, self.md, self.p, self.m, 1, amin, amax, self.ap, 0, self.surf_amp)
        ...
        rad_vel = giggle.urC(-gx, gy, self.ms, self.md, self.p, self.m, 1, amin, amax, self.ap, 0, self.surf_amp)
    ...
    def set_rt_grid(self):
        ...
        '''right now, these are still using adjusted phiind, need to verify this is actually what we want. Kind of makes sense because
        the surface density should also be offset by the same amount.'''
