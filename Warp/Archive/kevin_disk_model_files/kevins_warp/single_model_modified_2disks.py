...
from disk_modified import *
from raytrace_COMPLETE import *
from var_vis import *
...
'''
    ...
    chi = ((real_model-real_obj)**2*weight_real).sum() + ((imag_model-imag_obj)**2*weight_imag).sum()
    return chi
def waitup(filename, linelessfile):
    weight = var_vis(linelessfile,collapse=True) # JLI
    add_weight_to_vis(filename,weight) # JLI


def lnlike(p,highres=False,massprior=False,cleanup=False,systematic=False,line='co32',vcs=True,exp_temp=False,add_ring=False,use_galario=True):
    ...
    if line.lower() =='co21' or line.lower()=='co32' or line.lower()=='svco21':
        ...
        params = [p[0], #qq
                  0.032,#10**(p[1]), #Mdisk JLI
                  1.,#1.181526,#p[6],#1.,   #pp
                  1.,#10.,  #Rin
                  1000.,#Rout JLI
                  10**(p[1]),#10**(p[2]), #Rc
                  p[4], #different inclination. Should this be adjusted due to the multiple inclinations???? JLI
                  2.4, #k6,  #Mstar JLI
                  10**(-4.),#1e-4, #Xco
                  ...
                  [p[5],p[7]], #inner and outer boundaries for inner disk abundance JLI
                  1, #handed
                  [p[7]+p[8],1000.], #outer disk inner boundary for abundance JLI
                  p[9], #outer disk inclination JLI
                  p[10], #inner disk position angle JLI
                  p[11]] #outer disk position angle JLI
    if params[1] <0 or params[1]>params[7] or params[3]<0 or params[3]>params[4] or params[4]<0 or params[5]<0 or params[6]<0 or params[6]>90 or params[7]<0 or params[8]<0 or params[9]<0 or params[10]<0 or params[11]<0 or params[11]>params[12] or params[12]<0 or params[13][0]<0 or params[13][1]<0 or params[13][1]<params[13][0] or params[14][0]<0 or params[14][0]<params[3] or params[14][0]>params[14][1] or params[14][1]<0 or params[14][1]>params[4] or params[16][0]>params[16][1] or params[18]<0 or params[18]>360 or params[19]<0 or params[19]>360:
        chi = np.inf
        nu = 1
    else:
        if add_ring:
            if systematic:
                if p[-3]<0:
                    ...
                    disk_structure=Disk(params,rtg=False,exp_temp=exp_temp,ring=[(params[3]+p[-3])/2.,p[-3]-params[3],p[-2]]) #hm, iff add ring then we enter a conundrum where disk_modified's parameters don't exactly line up with the ones in lnlike. If this is used it'll require some adjustment JLI
            else:
                if p[-2]<0:
            ...
            disk_structure=Disk(params,rtg=False,exp_temp=exp_temp) #same here as above
            params2 = params[:] #JLI and most of the stuff below is also a Johnny addition
            params2[6] = params2[17]  #This makes the inclination in params2 to be the outer inclination first specified in p and in params JLI
            params2[14] = params2[16] #This makes the inner/outer boundaries for the disk correspond to the outer range.
            outer_disk_structure = Disk(params2,rtg=False,exp_temp=exp_temp)
            #The above code creates the two disk structures that we will be splicing together

            Rtrans = (p[6]+p[7])*disk_structure.AU #SUGAR okay so this has been knocking things around I think.

            disk_structure.set_rt_grid(vcs=vcs) #initialize the radiative transfer grid for the inner disk so it can be indexed and spliced with the outer
            outer_disk_structure.set_rt_grid(vcs=vcs) #initialize the radiative transfer grid for the outer disk so it can be indexed and spliced with the inner
            innerindex = disk_structure.r > Rtrans #innerindex tags the section of the disks that are to be removed/transplanted
            setattr(disk_structure,'inin',innerindex)
            #The above is for tagging which indicies to take from the inner disk

            #All these equalities below are for stitching the two together
            #Outer disk is the transplanter, inner disk is the transplantee
            comdensityG = disk_structure.rhoG #disk_structure is the inner disk's disk structure. The outer region will be cut out and the outer disk data will be pasted in where it was
            comtemperature = disk_structure.T
            comvelocity = disk_structure.Omg
            comXmol = disk_structure.Xmol
            comdBV = disk_structure.dBV
            comi_notdisk = disk_structure.i_notdisk
            comcs = disk_structure.cs

            comdensityG[disk_structure.inin] = outer_disk_structure.rhoG[disk_structure.inin]
            comtemperature[disk_structure.inin] = outer_disk_structure.T[disk_structure.inin]
            comtemperature[disk_structure.inin] = outer_disk_structure.T[disk_structure.inin]
            comvelocity[disk_structure.inin] = outer_disk_structure.Omg[disk_structure.inin]
            comXmol[disk_structure.inin] = outer_disk_structure.Xmol[disk_structure.inin]
            comdBV[disk_structure.inin] = outer_disk_structure.dBV[disk_structure.inin]
            comi_notdisk[disk_structure.inin] = outer_disk_structure.i_notdisk[disk_structure.inin]
            comcs[disk_structure.inin] = outer_disk_structure.cs[disk_structure.inin]

            setattr(disk_structure,'rhoG',comdensityG)
            setattr(disk_structure,'T', comtemperature)
            setattr(disk_structure,'Omg',comvelocity)
            setattr(disk_structure,'Xmol',comXmol)
            setattr(disk_structure,'dBV',comdBV)
            setattr(disk_structure,'i_notdisk',comi_notdisk)
            setattr(disk_structure,'cs',comcs)
            setattr(disk_structure, 'thet2',outer_disk_structure.thet)
            print('%r minutes: disks finished' % ((time.time()-start)/60.))

        if cleanup:
            #Use this if doing MCMC
            tf = tempfile.NamedTemporaryFile()
            modfile = tf.name[-9:]
            tf.close()
        else:
            #Use this if doing just a single model
            modfile = 'doubledisk'
        ...
        if line.lower() == 'co21':
            ...
            total_model(disk=disk_structure,chanmin=chanmin,nchans=nchans,chanstep=chanstep,offs=offs,modfile=modfile,imres=resolution,obsv=obsv,vsys=vsys, PA = p[9],freq0=230.538,Jnum=1,PAdos=p[10])
            if not use_galario:
                make_model_vis(datfile=datfile,modfile=modfile,isgas=True,freq0=230.538)
        #Only want co32 so we only care about the line data in these two bits below JLI
        #The below if is very specific to the CO3-2 line for HD100546 and will need to be modified if other data is used
        if line.lower() == 'co32':
            datfile = 'HD100546_co32_mid' #!!! MUST ALSO CHANGE MASK IN make_model and make_image FILES!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            hdr = fits.getheader(datfile+'.vis.fits')
            ...
            nchans = int(2*np.ceil(np.abs(obsv-vsys).max()/chanstep)+1)
            chanmin = -(nchans/2.-.5)*chanstep
            offs=[-0.4635,-0.18] #JLI This line has been un-gridsearchified. Add p[12],p[13] in to re-gridify With 0.02 steps, best fit is [-0.46,-0.18]
            resolution = 0.08 #Was at 0.05, kinda matters, kinda doesn't JLI
            obs = [150,101,300,150]
            #w = readsav('lowres_ALMA_weights_calc.sav')
            #new_weight = w['weight_alma']  JLI

            #disk_structure.set_obs(obs)
            #disk_structure.set_rt_grid(vcs = vcs)
            disk_structure.set_line(line) #JLI
            disk_structure.add_mol_ring(params[14][0],params[14][1],.79,3.,params[8],just_frozen=True) #JLI
            total_model(disk=disk_structure,chanmin=chanmin,nchans=nchans,chanstep=chanstep,offs=offs,modfile=modfile,freq0=345.79599,Jnum=2,imres=resolution,vsys=vsys,PA = p[10],obsv=obsv, PAdos = p[11])
            #disk_structure.plot_structure()
            print('%r minutes: post raytrace' % ((time.time()-start)/60.))
            if not use_galario:
        ...
        if not use_galario:
            ...
            chi = compare_vis_galario(datfile=datfile,modfile=modfile,systematic=sys,plot_resid=False)   #JLI

        if cleanup:
            # Clean up files
            files = [modfile+'.fits',modfile+'.han.im',modfile+'.im',modfile+'.model.vis',modfile+'.model.vis.fits']
            for file in files:
                os.system('rm -r '+file)
...
#            yoff_tot.append(yoff)
    ...
    yoff_tot=[]
    for ix in range(nsteps):
        kappa = -0.54+(-0.38+0.55)/nsteps*ix #JLI
        for iy in range(nsteps):
            gamma = -0.26+(-0.1+0.26)/nsteps*iy
            p=[-0.5,2,0,175,20,80,10,90,0,36,60,145,kappa,gamma]
            chi.append(-2*lnlike(p,cleanup=True))
