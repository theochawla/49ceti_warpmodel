...
def total_model(disk,imres=0.05,distance=110.,chanmin=-2.24,nchans=15,chanstep=0.32,flipme=True,Jnum=2,freq0=345.79599,xnpix=512,vsys=5.79,PA=146,offs=[0.150,0.05],modfile='model_visibility',abund=1.,obsv=None,wind=False,isgas=True,includeDust=False,extra=0,bin=1,hanning=False,PAdos = 126):
...
'''
    ...
    PAIN = PA
    POUT = PAdos
    iminner = ndimage.rotate(im2,90.+PAIN,reshape=False) #***#
    imouter = ndimage.rotate(im2,90.+POUT,reshape=False)

    PAIN = np.radians(PAIN)
    POUT = np.radians(POUT)

    #a = 200*disk.AU
    #b = a*np.cos(disk.thet)

    """
    As it stands right now the code is producing 100 x and y values for points on the perimeter of an eclipse of major axis a and minor axis b. The
    major axis should be the transition radius from inner to outer disk. Variables ex and wy should give these 100 points values of x and y in terms of pixels
    so that they can easily be tagged in the image.

    Conundrum 1
    - are ex and wy actually translating the radii in AU to pixel locations? Kinda confusing. im_s is a 512,512,15 array so the pixel locations should be in terms
    of 0-512, right? Look into whether or not this is actually the correct line of logic

    Conundrum 2
    - are these points necessary???? grabbing 100 different points doesn't help with all the rest of the points within the transition radius. Should ex and wy,
    instead of being 100 points based off of an angle from 0 to 2pi, instead be a list from 1 to 512 for each?? Seems like a looong thing to run. There has to
    be a faster way to tell it what the desired region is/cut down on the unnecessary bits, but I think that this is the right way to be thinking about this.

    Conundrum 3
    - actually editing those values in im_s. I guess that if they are in terms of its actual pixels (1 to 512) it shouldn't be all that hard to feed the image array
    the tagged indicies and to tell it to replace them with something else. Okay good. Looks like the real thing to deal with is conundrum 2. How do we make it faster?
    """

    #t = np.linspace(0, 2*np.pi,100)

    xaxis = np.arange(0,512)
    yaxis = np.arange(0,512)
    ym,xm = np.meshgrid(xaxis,yaxis)
    ...
    for pair in indie:
        ex = pair[0]
        wy = pair[1]
        Elipse = (((ex - 256)*np.cos(POUT) + (wy - 256)*np.sin(POUT))**2)/(a**2) + (((ex - 256)*np.sin(POUT) - (wy - 256)*np.cos(POUT))**2)/(b**2)
        if Elipse <= 1:
            imouter[ex,wy] = iminner[ex,wy]

    #ex = a*np.cos(t)*np.cos(PA) - b*np.sin(t)*np.sin(PA)
    #wy = a*np.cos(t)*np.sin(PA) + b*np.sin(t)*np.cos(PA)

    #print(ex)
    #print(wy)
    #print(Elipse, len(Elipse))

    #setattr(disk,'im_s',im_s) # JLI Testing Pop out


    pixshift = np.array([-1.,1.])*offs/(3600.*np.abs([hdr['cdelt1'],hdr['cdelt2']]))
    im_s = ndimage.shift(imouter,(pixshift[0],pixshift[1],0),mode='nearest')*Jy*(xpixscale/rad)**2
