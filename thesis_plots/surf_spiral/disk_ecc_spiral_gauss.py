...
#        tempg = tmid*np.exp(np.log(tatm/tmid)*zcf/zq)
        ...
        plt.pcolor(x_pol_grid, y_pol_grid, np.log10(siggas), cmap=cmocean.cm.thermal)
        plt.title("m=2")
        plt.colorbar(label="log10($M_\u2609$/($au^{2}$))")
        plt.xlabel("X, au")
        plt.ylabel("Y, au")
        fig.set_size_inches(4, 3.4)
        plt.savefig("surface_density_m2.jpg", bbox_inches='tight')
        plt.show()
